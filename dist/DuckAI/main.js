...
import { CustomModel, CustomModelInput, APIModeSelector, UserInput, ApiKeyInput, sendButton, NewChat, FreeUsage, showSpinner, showStatus, hideStatus, renderMessage, ApiKeyInputParent, LanguageSelector } from "./ui.js";
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    sendButton.addEventListener("click", async () => {
        ...
        showSpinner();
        try {
            const botMessage = await generate(history, FreeUsage?.checked ? null : apiKey, FreeUsage?.checked ? null : model, FreeUsage?.checked ? "OpenAI" : APIModeSelector.value.trim(), FreeUsage?.checked ?? false, LanguageSelector.value.trim());
            addMessage("assistant", botMessage);
            renderMessage("assistant", botMessage);
            hideStatus();
            const queryID = await storeConversation([{ role: "user", content: text }, { role: "assistant", content: botMessage }], CurrentDuckAIQueryID);
    ...
    document.getElementById("fileInput")?.addEventListener("change", async function () {
        ...
        if (!display || !this.files || !this.files.length)
            return;
