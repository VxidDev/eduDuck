...
export async function enhanceNotes(body: EnhanceNotesBody): Promise<EnhanceResponse> {
    const res = await fetch("/note-enhancer/enhance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export async function importNotes(file: File): Promise<ImportNotesResponse> {
    const formData = new FormData();
